  ...
  persist(
    (set, get) => ({
      ...
      removeDocument: (docId) => {
        ...
        // If the deleted doc was the "latest current", clear the ID
        if (docId === latestCurrentDocId) {
            set({ latestCurrentDocId: null });
        }
        
        // If the deleted doc was the currently viewed one, select the first of the remaining docs
        if (currentDoc?.id === docId) {
            set({ docs: newDocs, currentDoc: newDocs.length > 0 ? newDocs[0] : undefined });
        } else {
            set({ docs: newDocs });
        }
      },
